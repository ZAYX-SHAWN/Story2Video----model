...
# 配置中心：集中读取环境变量并设定默认值，便于生产环境注入和本地开发调�?
...
# 测试快速返回开关：开启后渲染接口直接返回占位视频路径，但后台仍执行真实生成流�?TEST_FAST_RETURN: bool = os.getenv("TEST_FAST_RETURN", "false").lower() in {"1", "true", "yes"}

SERVICE_PORT: int = int(os.getenv("SERVICE_PORT", "12345"))

# 初始化必要目�?OUTPUT_DIR.mkdir(exist_ok=True, parents=True)
